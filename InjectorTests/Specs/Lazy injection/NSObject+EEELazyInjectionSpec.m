...
describe(@"NSObject+EEELazyInjection", ^{
    ...
    context(@"class based properties", ^{
        ...
        beforeEach(^{
            ...
            injector.mapClass([NSString class]).toObject(@"Hello World!");
    ...
    it(@"does not override custom accessors", ^{
        TSTLazyInjectionCustom *sut = [[TSTLazyInjectionCustom alloc] init];
        sut.stringPropertyWithCustomAccessor = @"Hello";
        [[sut.stringPropertyWithCustomAccessor should] equal:@"Something"];
    });
});
